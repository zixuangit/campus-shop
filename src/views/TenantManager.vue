...
<script>
    export default {
        ...
        methods:{
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        axios.get('http://localhost:8181/tenant/search',{params:_this.ruleForm}).then(function (resp) {
            ...
            page(currentPage){
                ...
                if(_this.ruleForm.value == ''){
                    axios.get('http://localhost:8181/tenant/list/'+currentPage+'/'+_this.pageSize).then(function (resp) {
                        _this.tableData = resp.data.data.data
                        _this.total = resp.data.data.total
                    })
                } else {
            ...
            edit(row){
                this.$router.push('/tenantUpdate?id='+row.id)
            },
            del(row) {
                const _this = this
                this.$confirm('确认删除【'+row.name+'】吗？', '提示', {
                    ...
                    axios.delete('http://localhost:8181/tenant/deleteById/'+row.id).then(function (resp) {
                        if(resp.data.code==0){
        ...
        created() {
            if(this.$route.path=='/tenantManager2'){
                this.state=false
            }
            const _this = this
            axios.get('http://localhost:8181/tenant/list/1/'+_this.pageSize).then(function (resp) {
                _this.tableData = resp.data.data.data
                _this.total = resp.data.data.total
            })
        }
