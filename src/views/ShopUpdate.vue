    ...
    <div style="margin-top: 60px;margin-left:330px;width: 300px;height: 500px;border: 0px solid red;" >
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
            <el-form-item label="楼宇" prop="buildingId">
                <el-select v-model="ruleForm.buildingId" placeholder="请选择楼宇" disabled>
...
<script>
    export default {
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put('http://localhost:8181/shop/update',_this.ruleForm).then(function (resp) {
                            if(resp.data.code == 0){
        ...
        created(){
            const _this=this
            axios.get('http://localhost:8181/shop/findById/'+_this.$route.query.id).then(function (resp) {
                if(resp.data.code == 0){
