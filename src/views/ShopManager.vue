<template>
    <div style="margin-top: 60px;margin-left:80px;border: 0px solid red;" >
        ...
        <el-table
            ...
            <el-table-column label="操作" width="180" fixed="right" v-if="state">
...
<script>
    export default {
        ...
        methods:{
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        axios.get('http://localhost:8181/shop/search',{params:_this.ruleForm}).then(function (resp) {
            ...
            page(currentPage){
                ...
                if(_this.ruleForm.value == ''){
                    axios.get('http://localhost:8181/shop/list/'+currentPage+'/'+_this.pageSize).then(function (resp) {
                        _this.tableData = resp.data.data.data
                        _this.total = resp.data.data.total
                    })
                } else {
        ...
        created() {
            if(this.$route.path=='/shopManager2'){
                this.state=false
            }
            const _this = this
            axios.get('http://localhost:8181/shop/list/1/'+_this.pageSize).then(function (resp) {
                _this.tableData = resp.data.data.data
                _this.total = resp.data.data.total
            })
        }
